...
class Config:
    ...
    """
    local_path = '/Users/christophorus/Desktop/LeetCode'
    # solution of leetcode
    github_leetcode_url = 'https://github.com/ChristophorusX/LeetCode'
    # solution of pat,　暂时还没写
    github_pat_url = 'https://github.com/ChristophorusX/LeetCode'
                ...
                for _, _, files in os.walk(os.path.join(oj_algorithms, folder)):
                    ...
                    for item in files:
                        if item.endswith('.py'):
                            ...
                            self.table_item[
                                folder[:3]].python = '[Python]({})'.format(folder_url)
                            ...
                            folder_url = os.path.join(
                                Config.github_leetcode_url, folder_url)
                            self.table_item[
                                folder[:3]].java = '[Java]({})'.format(folder_url)
                            ...
                            folder_url = os.path.join(
                                Config.github_leetcode_url, folder_url)
                            self.table_item[
                                folder[:3]].c_plus_plus = '[C++]({})'.format(folder_url)
                            ...
                            folder_url = os.path.join(
                                Config.github_leetcode_url, folder_url)
                            self.table_item[
                                folder[:3]].javascript = '[JavaScript]({})'.format(folder_url)
        complete_num = sorted(complete_info.solved.items(
        ), key=lambda item_: item_[1], reverse=True)[0][1]
        readme = Readme(complete_info.total, complete_num,
                        complete_info.lock, complete_info.solved)
